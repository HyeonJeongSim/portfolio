...
{
    $is_html = "y";
    // HTML이 허용된 경우 위험한 태그만 필터링
    $subject = htmlspecialchars($subject);
    $content = strip_tags($content, '<p><br><strong><em><ul><li><a><img><h1><h2><h3><h4><h5><h6><blockquote><table><tr><td><th>');
}
else
{
    $is_html = "";
    // HTML이 허용되지 않은 경우 모든 태그를 엔티티로 변환
    $subject = htmlspecialchars($subject);
    $content = htmlspecialchars($content);
}
